...
const getPoolGenesisTxnPath = async function (poolName) {
  const tmpdir = FileSystem.documentDirectory;
  const dir = `${tmpdir}indy/`;
  const name = `${poolName}.txn`;

  await savePoolGenesisTxnFile(dir, name);
  return `${dir}${name}`.replace('file://', '');
};

const poolGenesisTxnData = function () {
  const poolIp = process.env.TEST_POOL_IP || '13.124.143.230';
...
const savePoolGenesisTxnFile = async function (dir, name) {
  const data = poolGenesisTxnData();
...
//       const folderPath = filePath.split('/').slice(0, filePath.split('/').length - 1).join('/');
...
const pathToIndyClientHome = function () {
  return FileSystem.documentDirectory + '/.indy_client';
}
